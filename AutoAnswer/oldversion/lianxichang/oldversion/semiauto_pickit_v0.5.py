...
def nextclass():
    ...
    savebutton = fullsc.crop(Button).save('tmp/Button.JPG','JPEG')
    picButtun = open('tmp/button.JPG','rb')
    Buttunmd5 = hashlib.md5(picButtun.read()).hexdigest().upper()
    picButtun.close()
...
def AutoAnswer():
    ...
    hashdict = pickle.load(output)			#此时hashdata已经是字典类型
        ##dict2
    outputv2 = open('hashdata_v2.pkl', 'rb')
    hashdict_v2 = pickle.load(outputv2)			#此时hashdata已经是字典类型
